...
describe("JwtAuthGuard", () => {
  ...
  let mockRequest: Partial<Writeable<AuthenticatedRequest>>;
  ...
  describe("canActivate", () => {
    ...
    it("should throw UnauthorizedException for token not yet valid", async () => {
      ...
      await expect(
        guard.canActivate(mockExecutionContext as unknown as ExecutionContext),
      ).rejects.toThrow("Access token not yet valid");
    });
  ...
  describe("error handling scenarios", () => {
    ...
    it("should handle concurrent authentication attempts correctly", async () => {
      ...
      expect((mockRequest1 as any).user).toEqual(payload1);
      expect((mockRequest2 as any).user).toEqual(payload2);
    });
