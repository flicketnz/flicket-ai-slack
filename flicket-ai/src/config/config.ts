  ...
  constructor() {
    ...
    this.slack = {
      ...
      logLevel: this.parseLogLevel(
        process.env.SLACK_LOG_LEVEL ?? this.logLevel,
      ),
    };
...
you MAY NOT use any heading syntax`,
  ...
  validate() {
    ...
    if (!this.assistant.searxngApiBase) {
      errors.push("SEARXNG_API_BASE is required");
    }
   ...
   * @returns
   */
  parseLogLevel(level: string): "DEBUG" | "INFO" | "WARN" | "ERROR" {
 ...
 */
export const appConfig = new Config() as Config & {
  llm: LLMConfig;
};
